// AXI_ooo.md Phase 3 -- the "teeth" build for the same-ID response-order checker.
// Identical to ../axi4_compile.f EXCEPT for the incdir below, which prepends the
// pre-Phase-2 (F2-buggy) slave driver proxy so VCS resolves
//   `include "axi4_slave_driver_proxy.sv"
// to it instead of to the fixed working-tree file. Everything else -- including
// the checker under test -- is the live tree. Iron rule 1: the working
// slave/axi4_slave_driver_proxy.sv is never patched to create a failure.
// Recreate the staged file with: bash mk_prephase2.sh   (see prephase2/README.md)
+incdir+./prephase2
+incdir+../../pkg/
+incdir+../../include/
