...
class axi4_master_monitor_proxy extends uvm_component;
  ...
  // values it hid a real routing error instead.
  //
  ...
  // WID and forbids write-data interleaving, so write data on one manager port
  // is always in address-issue order.
  //-------------------------------------------------------

  //Variable: axi4_master_write_data_pool
  //Combined write address+data packets awaiting their B response, keyed by AWID
  axi4_master_tx axi4_master_write_data_pool[int][$];
  uvm_event      axi4_master_write_data_pool_ev;

  //Variable: axi4_master_read_addr_pool
  //Read address packets awaiting their R burst, keyed by ARID
  axi4_master_tx axi4_master_read_addr_pool[int][$];
  uvm_event      axi4_master_read_addr_pool_ev;
  ...
  extern virtual task get_write_data_packet_by_id(input int id, output axi4_master_tx tx_h);
  extern virtual task get_read_addr_packet_by_id(input int id, output axi4_master_tx tx_h);
...
function axi4_master_monitor_proxy::new(string name = "axi4_master_monitor_proxy",
  ...
  axi4_master_write_data_pool_ev  = new("axi4_master_write_data_pool_ev");
  axi4_master_read_addr_pool_ev   = new("axi4_master_read_addr_pool_ev");
endfunction : new
...
task axi4_master_monitor_proxy::axi4_write_address();
  forever begin
    ...
    req_wr_clone_packet.sb_src_index = axi4_master_agent_cfg_h.master_id;
...
task axi4_master_monitor_proxy::axi4_write_data();
  forever begin
    ...
    axi4_master_write_data_pool[int'(req_wr.awid)].push_back(req_wr);
    axi4_master_write_data_pool_ev.trigger();
...
task axi4_master_monitor_proxy::axi4_write_response();
  forever begin
    ...
    master_tx_clone_packet.sb_src_index = axi4_master_agent_cfg_h.master_id;
...
task axi4_master_monitor_proxy::axi4_read_address();
  forever begin
    ...
    axi4_master_read_addr_pool[int'(req_rd.arid)].push_back(req_rd);
    axi4_master_read_addr_pool_ev.trigger();
    ...
    req_rd_clone_packet.sb_src_index = axi4_master_agent_cfg_h.master_id;
...
//--------------------------------------------------------------------------------------------
task axi4_master_monitor_proxy::get_write_data_packet_by_id(input int id, output axi4_master_tx tx_h);
  int waits = 0;
  while(!(axi4_master_write_data_pool.exists(id) && axi4_master_write_data_pool[id].size() > 0)) begin
    waits++;
    if(waits == unmatched_id_wait_limit) begin
      `uvm_warning("MASTER_MON_BID_UNMATCHED",$sformatf("B response with BID=%0d has waited through %0d other write completions with no outstanding write of that ID. A BID that was never issued, or an AxID truncated by the interconnect, looks exactly like this.",id,waits))
    end
    axi4_master_write_data_pool_ev.wait_trigger();
  end
  tx_h = axi4_master_write_data_pool[id].pop_front();
  if(axi4_master_write_data_pool[id].size() == 0) begin
    axi4_master_write_data_pool.delete(id);
  end
...
//--------------------------------------------------------------------------------------------
task axi4_master_monitor_proxy::get_read_addr_packet_by_id(input int id, output axi4_master_tx tx_h);
  int waits = 0;
  while(!(axi4_master_read_addr_pool.exists(id) && axi4_master_read_addr_pool[id].size() > 0)) begin
    waits++;
    if(waits == unmatched_id_wait_limit) begin
      `uvm_warning("MASTER_MON_RID_UNMATCHED",$sformatf("R burst with RID=%0d has waited through %0d other read requests with no outstanding read of that ID. An RID that was never issued, or an AxID truncated by the interconnect, looks exactly like this.",id,waits))
    end
    axi4_master_read_addr_pool_ev.wait_trigger();
  end
  tx_h = axi4_master_read_addr_pool[id].pop_front();
  if(axi4_master_read_addr_pool[id].size() == 0) begin
    axi4_master_read_addr_pool.delete(id);
  end
